            ...
            <x-jet-validation-errors class="mb-4" style="color: red"/>
                ...
                <form class="mt-4" method="POST" action="{{ route('updateUser', $profesor->id) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <div>
                            <select @if(auth()->user()->role_id == 2 || (auth()->user()->role_id == 1 && $profesor->role_id == 1 && auth()->user()->id != 1 && auth()->user()->id != $profesor->id) || auth()->user()->id == 1 && $profesor->id == 1) disabled @endif value="{{$profesor->name}}" name="role_id" x-model="role_id" class="block mt-1 w-full form-control border-gray-300 focus:border-indigo-300 focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
